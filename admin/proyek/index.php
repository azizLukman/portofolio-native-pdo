...
// Ambil data proyek dari yang terbaru
$query = mysqli_query($conn, "SELECT * FROM proyek ORDER BY id_proyek DESC");
?>
...
<head>
    ...
    <title>Dashboard Admin - Proyek</title>
...
<div class="container main-content">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            <div class="text-primary fw-bold small mb-1"><i class="fa-solid fa-code me-2"></i>MODUL PORTOFOLIO</div>
            <h1 class="fs-2 fw-bold m-0 text-white">DATA <span class="gradient-text">PROYEK</span></h1>
        </div>
        <div>
            <a href="tambah.php" class="btn-gradient-add">
                <i class="fa-solid fa-plus me-2"></i>Tambah Proyek Baru
            </a>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td><?= $no++; ?></td>
                            <td>
                                <div class="fw-bold text-white"><?= htmlspecialchars($data['nama_proyek'] ?? '') ?></div>
                            </td>
                            <td><?= htmlspecialchars($data['tech_stack'] ?? '-') ?></td>
                            <td>
                                <a href="<?= htmlspecialchars($data['link'] ?? '#') ?>" target="_blank" class="text-info text-decoration-none">
                                    <i class="fa-solid fa-external-link me-1"></i> Buka Link
                                </a>
                            </td>
                            <td class="text-center">
                                <?php 
                                $id_data = $data['id_proyek']; 
                                ...
                                echo '<a href="proses.php?aksi=hapus&id=' . $id_data . '" class="btn-action bg-danger bg-opacity-20 text-danger border border-danger border-opacity-20" title="Hapus" onclick="return confirm(\'Apakah Anda yakin ingin menghapus proyek ini?\')"><i class="fa-solid fa-trash-can"></i></a>';
                                ?>
                            </td>
                        </tr>
                    <?php 
                    ...
                    ?>
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted">
                                ...
                                Belum ada data proyek yang tersimpan.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
